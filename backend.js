...
function startup(){
	...
	if ( assignmentId == 'ASSIGNMENT_ID_NOT_AVAILABLE' ){
	    ...
	    // Option for php, gets task based on the worker
	    if (gup('mode') == 'DATABASE'){
		if (file_name == 'NO_VAL' && workerId != 'NO_VAL'){
			...
			alert('Error, this mode only exists with php')
		    }
		}
	    }
	    else if (gup('mode') == 'TASK_NAME'){
		$("#startHIT").css('display', 'none');
		var str = ""
		time_init = new Date();
	    ...
	    else if (gup('mode') == 'TASK_FILE_NAME'){
		$("#startHIT").css('display', 'none');
		var str = ""
		time_init = new Date();
		$('#action_ad').click(function() {
		...
		$.getScript('example_tasks/'+file_name+'.js', function(data, textstatus, jqxhr){
		    for (var tid = 0; tid < task_name.length; tid++){
			createCell(task_name[tid])
		    }
		});
